...
def predict(model, noisy_dataset, gt_dataset, device, padding, n_channels, results_path):
    ...
    if results_path is not None:
        for i in range(n_images):
            ...
            y_img = (255 * y_orig[i]).astype('uint8')
            x_img = (255 * xs[i]).astype('uint8')
            ...
            name = os.path.join(results_path, '{}_original_image.png'.format(i))
            io.imsave(name, y_img)

            name = os.path.join(results_path, '{}_noisy_image.png'.format(i))
            io.imsave(name, x_img)
